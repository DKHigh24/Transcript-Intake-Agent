...
- [ ] Add 4 review fields to `config/mvp_output_schema.json`:
  `review_status` (null), `reviewer_id` (null), `reviewer_timestamp` (null), `reviewer_notes` (null)
- [ ] Add `_model` snapshot field to `config/mvp_output_schema.json` (object, null)
- [ ] Bump `schema_version` from 1 to 2 in `config/mvp_output_schema.json`
- [ ] Add `ReviewStatus` choice list to `config/choice_values.json`:
  `["approved", "rejected", "needs_reprocess", "merged"]`
- [ ] Add `REVIEWER_ID` to `.env.example` with comment

### 1.2 Schema migration module

- [ ] Create `src/schema_migrations.py`:
  - `CURRENT_VERSION = 2`
...
- [ ] In `src/classifier.py`, after parsing JSON from the LLM:
...
- [ ] Add `"review"` to `MODES` list in `main.py`
- [ ] Create `run_review(date: str | None)` function in `main.py`:
...
- [ ] Add `review_status` column to `REVIEW_COLUMNS` list in `src/review_exporter.py`
  (between `SignalScore` and `_validation_warnings`)
- [ ] Add conditional cell fill: green for approved, red for rejected, yellow for pending
...
- [ ] In `review_queue.py`, after each action:
...
- [ ] Create `src/feedback_applier.py`:
...
- [ ] Add `"apply-feedback"` to `MODES`
- [ ] Create `run_apply_feedback(version: str | None)` in `main.py`:
  - Load feedback log
...
- [ ] Add `"promote-feedback"` to `MODES`
- [ ] Add `--feedback-version` argument to argparser
- [ ] Create `run_promote_feedback(version: str)` in `main.py`:
...
- [ ] In `src/report_generator.py`, `_build_cards_html()`:
...
- [ ] In `src/trend_reporter.py`, `generate_weekly_report()`:
...
- [ ] Run `--mode weekly` on a test transcript — confirm queue.json is written
- [ ] Run `--mode review` — confirm interactive CLI shows items and saves actions
- [ ] Run `--mode weekly --push-ado` with no approvals — confirm zero items pushed, warning shown
- [ ] Run `--mode weekly --push-ado` with some approved — confirm only approved items pushed
- [ ] Run `--mode weekly --push-ado --skip-review-gate` — confirm all primary rows pushed with warning
...
- [ ] Load an old archived `classified_rows.json` (pre-migration) — confirm no error, null review fields added
